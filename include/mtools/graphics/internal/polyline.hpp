...
namespace internals_polyline
	...
	/** sub routine */
	MTOOLS_FORCEINLINE void _enlargeline(const fVec2 & P1, const fVec2 & P2, const fVec2 & P3, double thick, fVec2 & A1, fVec2 & A2, fVec2 & A3, fVec2 & B1, fVec2 & B2, fVec2 & B3)
		{
		fVec2 H1 = (P2 - P1).get_rotate90(); H1.normalize(); H1 *= thick;
		fVec2 H2 = (P3 - P2).get_rotate90(); H2.normalize(); H2 *= thick;
		A1 = P1 + H1; const fVec2 A1b = A1 + (P2 - P1);
		A3 = P3 + H2; const fVec2 A3b = A3 + (P2 - P3);
		if (!intersection(A1, A1b, A3, A3b, A2)) { MTOOLS_DEBUG("no intersection for A2");  A2 = 0.5*((A1b - P2) + (A3b - P2)); } // approximation...
		B1 = P1 - H1; const fVec2 B1b = B1 - (P2 - P1);
		B3 = P3 - H2; const fVec2 B3b = B3 - (P2 - P3);
	...
	*/
	inline void polylinetoPolygon(const std::vector<fVec2> & tabPoints, double thickness, std::vector<fVec2> & tabout)
		...
		for (size_t i = 1; i < l - 1; i++)
			{
			fVec2 A1, A3, B1, B3;
			_enlargeline(tabPoints[i - 1], tabPoints[i], tabPoints[i + 1], thickness, A1, tabout[i], A3, B1, tabout[N-i], B3);
	...
	/**
	 * Enlarge a polyline by a given thickness.
	 *
	 * @param 		  	tabPoints points that constitute the polyline
	 * @param 		  	thickness line thickness
	...
	inline void enlargePolyline(const std::vector<fVec2> & tabPoints, double thickness, std::vector<fVec2> & tabA, std::vector<fVec2> & tabB)
		...
		for (size_t i = 1; i < l - 1; i++)
			{
			fVec2 A1, A3, B1, B3;
			_enlargeline(tabPoints[i - 1], tabPoints[i], tabPoints[i + 1], thickness, A1, tabA[i], A3, B1, tabB[i], B3);
	...
	*/
	inline void enlargePolygon(const std::vector<fVec2> & tabPoints, double thickness, std::vector<fVec2> & tabA, std::vector<fVec2> & tabB)
		...
		fVec2 A1, A3, B1, B3;
		_enlargeline(tabPoints[l - 1], tabPoints[0], tabPoints[1], thickness, A1, tabA[0], A3, B1, tabB[0], B3);
		_enlargeline(tabPoints[l - 2], tabPoints[l - 1], tabPoints[0], thickness, A1, tabA[l - 1], A3, B1, tabB[l - 1], B3);
		}
