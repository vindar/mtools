...
Img<unsigned char> * coulF(iVec2 pos, iVec2 imSize, void * data)
    {
    static Img<unsigned char> im(1, 1, 1, 4);
    if ((imSize.X() == 1) && (imSize.Y() == 1)) 
        { 
        if ((pos.X() >0)&&(pos.Y()>0)) im.setPixel({ 0,0 }, RGBc::c_Green); 
        else return nullptr;
        }
    else
        {
        if ((pos.X() >0) && (pos.Y()>0)) im.setPixel({ 0,0 }, RGBc::c_Red);
        else im.setPixel({ 0,0 }, RGBc::c_Black);
        }
    return &im;
    }
...
class TreeEdenCluster
    ...
    public:
        ...
        void simulate(size_t steps)
            ...
            while (N < finalN)
                ...
                if (S.direction == 1)
                    ...
                    N++;
                    S.N = N;
                    iVec2 upPos(pos.X(), pos.Y() + 1); auto & up = Grid(upPos); if (up.N > 0) { S.direction = 1; }
                    else { if ((up.direction)++ == 0) Urn.insert(upPos); }
                    iVec2 downPos(pos.X(), pos.Y() - 1); auto & down = Grid(downPos); if (down.N > 0) { S.direction = 2; }
                    else { if ((down.direction)++ == 0) Urn.insert(downPos); }
                    iVec2 leftPos(pos.X() - 1, pos.Y()); auto & left = Grid(leftPos); if (left.N > 0) { S.direction = 3; }
                    else { if ((left.direction)++ == 0) Urn.insert(leftPos); }
                    iVec2 rightPos(pos.X() + 1, pos.Y()); auto & right = Grid(rightPos); if (right.N > 0) { S.direction = 4; }
                    else { if ((right.direction)++ == 0) Urn.insert(rightPos); }
                    }
                }
            }
        ...
        Img<unsigned char> * getImage(mtools::iVec2 pos, mtools::iVec2 size)
            {
            auto v = Grid.peek(pos);
...
void run()
    ...
    auto P = makePlot2DLattice(EC, "Tree Eden model"); P.opacity(0.5f);
    P.setImageType(P.TYPEIMAGE);
    Plotter2D Plotter;

    auto F = makePlot2DLattice(coulF, "Test func"); F.opacity(0.3f);
    Plotter[P][F];
    Plotter.startPlot();
